...
class TrainingExamListAdapter(private val mContext: Context, private val trainingExams: MutableList<TrainingExam>, private val listener: TrainingExamListAdapterListener) : RecyclerView.Adapter<TrainingExamListAdapter.ListHolder>() {
    ...
    override fun onBindViewHolder(holder: ListHolder, position: Int) {
        ...
        holder.from.text = exam.title
        holder.subject.text = exam.country
        holder.message.setText(String.format("Passmark ", exam.passmark))
        holder.timestamp.text = exam.dateCreated

        // displaying the first letter of From in icon text
        holder.iconText.setText(exam.title.substring(0, 1))
    ...
    private fun applyIconColor(holder: ListHolder, exam: TrainingExam) {
        holder.imgProfile.setImageResource(R.drawable.bg_circle)
        holder.imgProfile.setColorFilter(exam.color)
    ...
    private fun applyImportant(holder: ListHolder, exam: TrainingExam) {
        if (exam.country.equals("KE", ignoreCase = true)) {
    ...
    fun removeData(position: Int) {
        trainingExams.removeAt(position)
        notifyItemRemoved(position)
        resetCurrentIndex()
    }

    fun restoreItem(exam: TrainingExam, position: Int) {
    ...
    interface TrainingExamListAdapterListener {
